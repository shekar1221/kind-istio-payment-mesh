...
- Kubernetes finalizers with a custom `PaymentCleanup` CRD and controller
- Normal deletion, deliberately stuck deletion, safe recovery and finalizer interview Q&A
...
k8s/finalizers/             CRD, RBAC, controller and finalizer demos
...
10. **Finalizers:** deploy a custom controller, observe `deletionTimestamp`, verify cleanup, reproduce a stuck object and recover safely.
...
## Optional finalizers lab

After the base payment application is running:
...
The finalizer controller adds `payments.example.com/archive` to a custom `PaymentCleanup` resource. During deletion it writes simulated audit evidence, removes only its own finalizer, and allows the API server to finish deletion. The stuck lab uses an unowned finalizer to demonstrate safe diagnosis and targeted recovery.

Read [docs/INSTALLATION.md](docs/INSTALLATION.md) for detailed setup, [docs/INTERVIEW-QA.md](docs/INTERVIEW-QA.md) for Istio interview preparation, [docs/FINALIZERS-LAB.md](docs/FINALIZERS-LAB.md) for the finalizer workshop, and [docs/FINALIZERS-INTERVIEW-QA.md](docs/FINALIZERS-INTERVIEW-QA.md) for finalizer questions and answers.
